...
export async function fetchTeamSchedules(apiKey, year) {
  ...
  for (const team of trackedTeams) {
    ...
    // CFBD occasionally tags a team's late-August opener with the same
    // week number as its following game (e.g. both coming back as
    // "week 1"). Walking backward and pushing any such collision down
    // (week 1, week 1 -> week 0, week 1) keeps each team's own week
    // numbers unique and chronological without touching the rest of
    // the schedule.
    for (let i = teamGames.length - 2; i >= 0; i--) {
      if (teamGames[i].week >= teamGames[i + 1].week) {
        teamGames[i].week = teamGames[i + 1].week - 1
      }
    }
  }
