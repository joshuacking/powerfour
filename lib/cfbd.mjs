...
export async function fetchTeamSchedules(apiKey, year) {
  ...
  // CFBD tags each season's early-arriving openers (teams playing the
  // weekend before the main slate, e.g. USC-San José State) as "week 1"
  // just like the following weekend's games, so a team can end up with
  // two week-1 entries. The season's real week 1 games start the first
  // Thursday of September; anything CFBD calls week 1 that kicks off
  // before that is actually last week's game, so relabel it week 0.
  const week1Start = Date.UTC(year, 8, 3)
  for (const team of trackedTeams) {
    const teamGames = schedules[team]
    teamGames.sort((a, b) => new Date(a.startDate) - new Date(b.startDate))
    for (const game of teamGames) {
      if (game.week === 1 && new Date(game.startDate).getTime() < week1Start) {
        game.week = 0
      }
    }
  }
