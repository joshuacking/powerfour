...
// of "North Carolina"), so it needs prefix matching against our tracked
// team names rather than an exact match. Sorted longest-name-first so
// e.g. "Texas A&M" is matched before the shorter "Texas" prefix.
const TRACKED_TEAM_MATCHERS = [...trackedTeams]
  .map((team) => ({ team, cfbdName: CFBD_NAME_ALIASES[team] ?? team }))
  .sort((a, b) => b.cfbdName.length - a.cfbdName.length)

function matchTrackedTeamName(fullName) {
  for (const { team, cfbdName } of TRACKED_TEAM_MATCHERS) {
    if (fullName === cfbdName || fullName.startsWith(`${cfbdName} `)) {
      return team
    }
  }
  return null
...
export async function fetchScoreboard(apiKey, year) {
  ...
  for (const g of games) {
    for (const [teamSide, opponentSide] of [
      ...
      const team = matchTrackedTeamName(teamSide.name)
      if (!team) continue
      scoreboard[team] = {
        status: g.status,
        period: g.period,
