// Fetches team schedules from the College Football Data API
// (https://api.collegefootballdata.com) and writes them to a static
// JSON file consumed by the app. Team records are fetched live at
// request time instead (see api/records.js), since they change
// throughout the season.
//
// Run with:
//   node --env-file=.env scripts/fetch-schedules.mjs

import { writeFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import path from 'node:path'
import { cfbdGet, fromCfbdName, trackedTeams } from '../lib/cfbd.mjs'
...
if (!API_KEY) {
  console.error(
    'Missing CFBD_API_KEY. Run with: node --env-file=.env scripts/fetch-schedules.mjs',
  )
  process.exit(1)
}
